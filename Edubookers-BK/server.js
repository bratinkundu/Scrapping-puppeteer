...
var getProviderData = async (page) => {
    var providerData = await page.evaluate(async () => {
        ...
        var themes = [] 
        var themeElement = document.querySelectorAll('div.Course-detail__features > ul')[1].querySelectorAll('li')
        if(themeElement){
            themeElement.forEach(el => {
                var theme = el.querySelector('span.Tag__text').innerText.trim();
                themes.push(theme);
            })
        }
    ...
    if(moreProductButton){
        ...
        var AllProducts = await page.evaluate(()=>{
            ...
            document.querySelectorAll('#Category-list__results > article').forEach(el => {
                products.push(el.querySelector('a.Course__name').innerText.trim())
            })
    ...
    for (let i = 0; i < AllProducts.length; i++){
        ...
        var productData = await page.evaluate(async ()=>{
            ...
            AllStartDates.forEach(async (ele) => {
                ...
                var price = ele.querySelector('header > div.Price').innerText.match(/\d/g).join("");
                var lessons = []
                ...
                var totalPrice = ele.querySelector('div.Expandable__item > div.Course-calendar__cta > table.Price-details > tfoot > tr > td > div') ? ele.querySelector('div.Expandable__item > div.Course-calendar__cta > table.Price-details > tfoot > tr > td > div').innerText.replace(',','.').split('\n')[0].split(' ')[1] : ""
